...
CRD_PATH             := $(if $(CRD_PATH),$(CRD_PATH),crds)
# https://github.com/appscodelabs/gengo-builder
CODE_GENERATOR_IMAGE ?= appscode/gengo:release-1.18

API_GROUPS           ?= $(subst /,:, $(shell find $$(pwd)/apis -maxdepth 2 -mindepth 2 -type d -name 'v*' -printf '%P '))
API_GROUPS_BASE      ?= $(filter base%, $(API_GROUPS))
API_GROUPS_NONBASE   ?= $(filter-out base%, $(API_GROUPS))
...
ifdef git_tag
	...
	ifeq (,$(findstring $(git_branch),master HEAD))
		ifneq (,$(patsubst release-%,,$(git_branch)))
			VERSION := $(subst /,-,$(git_branch))-$(commit_hash_short)
...
gen-types: $(BUILD_DIRS)
	...
	@docker run                                                 \
	    ...
	    /bin/bash -c "                                          \
	        ...
	        ./hack/run.sh                                       \
	    "
	@echo "Making fmt add-license..."
	@$(MAKE) fmt add-license --no-print-directory
